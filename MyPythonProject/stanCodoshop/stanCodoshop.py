...
def get_best_pixel(pixels):
    ...
    pixels_dic = []
    for i in pixels:
        pixels_dic.append(get_pixel_dist(i, pixels_avg[0], pixels_avg[1], pixels_avg[2]))
    return pixels[pixels_dic.index(min(pixels_dic))]
...
def solve(images):
    ...
    for x in range(result.width):
        for y in range(result.height):
            img_pixels = []
            for i in images:
                img_pixels.append(i.get_pixel(x, y))
            pixel = get_best_pixel(img_pixels)
