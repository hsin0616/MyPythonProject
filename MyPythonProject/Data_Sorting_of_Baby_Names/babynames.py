...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        if year in name_data[name]:
            ...
            new_rank = int(rank)
            name_data[name][year] = str(min(existing_rank, new_rank))
        else:
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        # 將結果賦值給變數 year
        year = f.readline().strip()  # strip() is for 刪除行末的換行符號
        for line in f:
            rank, name1, name2 = line.strip().split(',')
            # strip() is for  刪除行首和行末的空白字符
            # split(',') is for 將文字根據逗號 , 分割成一個包含三個元素的列表
            rank = rank.strip()
            name1 = name1.strip()
            name2 = name2.strip()
